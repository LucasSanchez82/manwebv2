...
const ProfileEditorForm = () => {
    ...
    const fieldNames = Object.keys(profileSchemaForm.shape) as keyOfProfileSchemaShape[];
    ...
    return (
        <AutoForm formSchema={profileSchemaForm} fieldConfig={{
            
            email: {inputProps: {defaultValue: session?.user?.email || undefined}},
            
            name: {inputProps: {defaultValue: session?.user?.name || undefined}},
            
            image: {inputProps: {defaultValue: session?.user?.image || undefined}},
        }}>
            <SheetFooter>
